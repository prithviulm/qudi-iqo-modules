...
01. Create a release branch eg, `release_vX.0.0` and collect all
    needed changes in here.

02. Check `qudi-iqo-modules/setup.py` and fix all versions with `==`
    according to the versions of the test setup mentioned above.

03. Test whether the installation runs flawlessly. To this end, create a
...
04. Update `qudi-iqo-modules/docs/changelog.md`. All collected
...
05. Edit `qudi-iqo-modules/.github/ISSUE_TEMPLATE/bug_report.yaml` and
    add the option to choose the release version that you just released.

06. Edit the Troubleshooting section in
    `qudi-iqo-modules/docs/installation_guide.md` to point at
    installing the latest release. Eg. `git checkout tags/v0.4.0`

07. Only after testing, create a PR and merge `release_vX.0.0` into
    `main`.

08. Increment version number in `qudi-iqo-modules/VERSIONS`. This will
    already trigger a release from the main branch to test.pypi.

09. Execute the release by navigating in github to qudi-iqo-modules. On
