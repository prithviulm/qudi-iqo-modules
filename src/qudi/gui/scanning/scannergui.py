...
class ConfocalMainWindow(QtWidgets.QMainWindow):
    ...
    def mouseDoubleClickEvent(self, event):
        if event.button() == QtCore.Qt.LeftButton:
            self.action_utility_zoom.setChecked(
                not self.action_utility_zoom.isChecked()
            )
            event.accept()
        else:
...
class ScannerGui(GuiBase):
    ...
    _optimize_logic = Connector(
        name="optimize_logic", interface="ScanningOptimizeLogic"
    )

    # config options for gui
    _default_position_unit_prefix = ConfigOption(
        name="default_position_unit_prefix", default=None
    )
    # for all optimizer sub widgets, (2= xy, 1=z)
    _optimizer_plot_dims = ConfigOption(
        name="optimizer_plot_dimensions", default=[2, 1]
    )
    # minimum crosshair size as fraction of the displayed scan range
    _min_crosshair_size_fraction = ConfigOption(
        name="min_crosshair_size_fraction", default=1 / 50, missing="nothing"
    )
    ...
    def on_activate(self):
        ...
        self._mw.action_restore_default_view.triggered.connect(
            self.restore_default_view
        )
        self._mw.action_save_all_scans.triggered.connect(
            lambda x: self.save_scan_data(scan_axes=None)
        )
        self.sigSaveScan.connect(
            self._data_logic().save_scan_by_axis, QtCore.Qt.QueuedConnection
        )
    ...
    def on_deactivate(self):
        ...
        self._optimize_logic().sigOptimizeStateChanged.disconnect(
            self.optimize_state_updated
        )
        self._data_logic().sigHistoryScanDataRestored.disconnect(
    ...
    def _init_static_dockwidgets(self):
        ...
        self._mw.addDockWidget(
            QtCore.Qt.TopDockWidgetArea, self.scanner_control_dockwidget
        )
        ...
        self._mw.action_view_line_scan.triggered[bool].connect(
            lambda is_vis: [
                wid.setVisible(is_vis) for wid in self.scan_1d_dockwidgets.values()
            ]
        )
        self.scanner_control_dockwidget.sigResolutionChanged.connect(
            lambda ax, res: (
                self.sigScanSettingsChanged.emit({"resolution": {ax: res}})
        ...
        self.scanner_control_dockwidget.sigRangeChanged.connect(
            lambda ax, ranges: (
                self.sigScanSettingsChanged.emit({"range": {ax: ranges}})
                if not self._scanner_settings_locked
                else None
            )
        )
        ...
        self._mw.action_view_toolbar.triggered[bool].connect(
            self._mw.util_toolBar.setVisible
        )

        # Add tilt correction widget to the toolbar as a button
        self.tilt_correction_dockwidget = TiltCorrectionDockWidget(
            scanner_axes=self._scanning_logic().scanner_axes
        )
        self.tilt_correction_dockwidget.setAllowedAreas(QtCore.Qt.BottomDockWidgetArea)
        self._mw.addDockWidget(
            QtCore.Qt.BottomDockWidgetArea, self.tilt_correction_dockwidget
        )
    ...
    def restore_default_view(self):
        ...
        self._mw.addDockWidget(
            QtCore.Qt.BottomDockWidgetArea, self.scanner_control_dockwidget
        )
        ...
        if has_1d_scans and has_2d_scans:
            self._mw.splitDockWidget(
                dockwidgets_1d[0], self.optimizer_dockwidget, QtCore.Qt.Vertical
            )
            self._mw.resizeDocks(
                (dockwidgets_1d[0], self.optimizer_dockwidget),
                (3, 2),
                QtCore.Qt.Vertical,
            )
            self._mw.resizeDocks(
                (dockwidgets_2d[0], dockwidgets_1d[0]), (1, 1), QtCore.Qt.Horizontal
            )
        elif multiple_2d_scans:
            self._mw.splitDockWidget(
                dockwidgets_2d[1], self.optimizer_dockwidget, QtCore.Qt.Vertical
            )
            self._mw.resizeDocks(
                (dockwidgets_2d[1], self.optimizer_dockwidget),
                (3, 2),
                QtCore.Qt.Vertical,
            )
            self._mw.resizeDocks(
                (dockwidgets_2d[0], dockwidgets_2d[1]), (1, 1), QtCore.Qt.Horizontal
            )
        elif has_1d_scans:
            self._mw.resizeDocks(
                (dockwidgets_1d[0], self.optimizer_dockwidget),
            ...
            self._mw.resizeDocks(
                (dockwidgets_2d[0], self.optimizer_dockwidget),
                (1, 1),
                QtCore.Qt.Horizontal,
            )
    ...
    def save_scan_data(self, scan_axes=None):
        ...
        ----------
        scan_axes : tuple
            Axis to save. Save all currently displayed if None.
        """
        ...
        try:
            ...
            if scan_axes is None:
                scan_axes = [
                    scan.scan_axes for scan in data_logic.get_all_current_scan_data()
                ]
            else:
                scan_axes = [scan_axes]
            for ax in scan_axes:
                try:
                    cbar_range = self.scan_2d_dockwidgets[
                        ax
                    ].scan_widget.image_widget.levels
                except KeyError:
                    cbar_range = None
    ...
    def _add_scan_dockwidget(self, axes):
        ...
        if len(axes) == 1:
            ...
            dockwidget.scan_widget.set_plot_range(
                x_range=axes_constr[0].value_range, y_range=axes_constr[1].value_range
            )
    ...
    def apply_scanner_settings(self):
        ...
        forward_freq = {
            ax: freq[0] for ax, freq in self._ssd.settings_widget.frequency.items()
        }
    ...
    def scanner_settings_updated(self, settings=None):
        ...
        ----------
        settings : dict
            Settings dict containing the scanner settings to update.
            If None (default), read the scanner settings from logic and update.
        """
        ...
        if "frequency" in settings:
            ...
            new_freq = {
                ax: (forward, old_freq[ax][1])
                for ax, forward in settings["frequency"].items()
            }
    ...
    def set_scanner_target_position(self, target_pos):
        """
        Issues new target to logic and updates the GUI.
    ...
    def scanner_target_updated(self, pos_dict=None, caller_id=None):
        """
        Updates the scanner target and sets widgets accordingly.

        Parameters
        ----------
        pos_dict : dict, optional
            The scanner position dict to update each axis position. If None (default),
            read the scanner position from logic and update.
        caller_id : int
    ...
    def toggle_optimize(self, enabled):
        """ """
        self._toggle_enable_actions(
            not enabled, exclude_action=self._mw.action_optimize_position
        )
    ...
    def restore_history(self):
        ...
        restored_axs = []
        ids_to_restore = np.asarray(
            [self._data_logic().get_current_scan_id(ax) for ax in avail_axs]
        )
    ...
    def _update_scan_markers(self, pos_dict, exclude_scan=None):
        ...
        for scan_axes, dockwidget in self.scan_2d_dockwidgets.items():
            if exclude_scan != scan_axes:
                ...
                new_pos = (
                    pos_dict.get(scan_axes[0], old_x),
                    pos_dict.get(scan_axes[1], old_y),
                )
        ...
        for scan_axes, dockwidget in self.scan_1d_dockwidgets.items():
            if exclude_scan != scan_axes:
                new_pos = pos_dict.get(
                    scan_axes[0], dockwidget.scan_widget.marker_position
                )
                dockwidget.scan_widget.blockSignals(True)
                dockwidget.scan_widget.set_marker_position(new_pos)
                dockwidget.scan_widget.blockSignals(False)
        ...
        if len(axes) == 2:
            ...
            def set_range_func(x_range, y_range):
                ...
                self.sigScanSettingsChanged.emit(
                    {"range": {axes[0]: x_range, axes[1]: y_range}}
                )
    ...
    def update_optimizer_settings(self, settings=None):
        ...
        if "scan_sequence" in settings:
            new_settings = self._optimize_logic().check_sanity_optimizer_settings(
                settings, self._optimizer_plot_dims
            )
    ...
    def tilt_corr_support_vector_updated(self, settings):
        """
        Signal new vectors from logic and update the GUI accordingly.
        ...
        if settings:
            sup_vecs = np.asarray(
                [settings["vec_1"], settings["vec_2"], settings["vec_3"]]
            )
    ...
    def apply_tilt_corr_support_vectors(self):
        ...
        dim_idxs = [
            (idx, key)
            for idx, key in enumerate(self._scanning_logic().scanner_axes.keys())
        ]

        all_vecs_valid = True
        vecs_to_check = (
            [0, 1, 2] if self.tilt_correction_dockwidget.auto_origin else [0, 1, 2, 3]
        )
