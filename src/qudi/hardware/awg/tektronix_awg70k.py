...
class AWG70K(PulserInterface):
    ...
    load_dict : dict or list
        A dictionary with keys being one of the available channel indexes and values being the name of the already written
        waveform to load into the channel.
        Examples:
            {1: 'rabi_ch1', 2: 'rabi_ch2'} or {1: 'rabi_ch2', 2: 'rabi_ch1'}.
        If just a list of waveform names is given, the channel association will be invoked from the channel
    ...
    dict
        ...
        written with `write_waveform` ready to play.
    
    Notes
    -----
    The channel index used here is not to be confused with the number suffix
    ...
    being associated with a SINGLE waveform asset.
    """
    ...
    workspace into the channels. For a device without mass memory, this will make the waveform/pattern that has been
    previously written with `write_waveform` ready to play.
    
    Parameters
    ----------
    sequence_name : str
        The name of the sequence to be loaded.
    ...
    ------
    ValueError
        If the sequence to load is missing in device memory or if the number of sequence tracks does not
        match the number of active analog channels.
    
    Notes
    -----
    The channel association will be invoked from the channel suffix '_ch1', '_ch2', etc.
    """
    ...
    -------
    tuple
        (int, dict): Integer value of the current status with the
        corresponding dictionary containing status description for all the possible status variables
    ...
    ----------
    sample_rate : float
        The sample rate to be set (in Hz).

    Returns
    -------
    float
        The sample rate returned from the device (-1:error).
    """
    ...
    def get_sample_rate(self):
        """
    Get the current sample rate of the pulse generator hardware.
    ...
    offset : list, optional
        If a specific high value (in Volt) of a channel is desired.

    Returns
    -------
    dict
        with keys being the generic string channel names and items
    ...
    amplitude : dict, optional
        Dictionary with key being the channel and items being the amplitude values (in Volt peak to peak)
        for the desired channel.
    offset : dict, optional
        Dictionary with key being the channel and items being the offset values (in absolute volt) for the
        desired channel.
    ...
    ----------
    low : list, optional
        If a specific low value (in Volt) of a channel is desired.
    high : list, optional
        If a specific high value (in Volt) of a channel is desired.
    ...
    -------
    tuple
        (dict, dict): Tuple of two dicts, with keys being the channel
        number and items being the values for those channels. Both low and high value of a channel is
    ...
    ----------
    low : dict, optional
        Dictionary with key being the channel and items being the low values (in volt) for the desired channel.
    high : dict, optional
        Dictionary with key being the channel and items being the high values (in volt) for the desired channel.

    Returns
    -------
    tuple
        (dict, dict): Tuple of two dicts where first dict denotes the
        current low value and the second dict the high value.
    """
    ...
    ----------
    ch : list, optional
        If specific analog or digital channels are needed to be asked without obtaining all the channels.

    Returns
    -------
    dict
        where keys denoting the channel number and items boolean
    ...
    ----------
    state : bool
        The state the interleave should be set to (True for ON, False for OFF).

    Returns
    -------
    bool
        The actual interleave status (True if ON, False if OFF).
    ...
    -------
    int
        Error code (0 for OK, -1 for error).
    """
    ...
    ----------
    name : str
        Name of the sequence which should be generated.
    steps : int
        Number of steps.
    ...
    ----------
    sequence_name : str
        Name of the sequence which should be edited.
    waveform_name : str
    ...
    repeat : int
        Number of repetitions. (-1: infinite, 0: once, 1: twice, ...)
    ...
    step : int
        Sequence step to be edited.
    trigger : str
        Trigger string specifier. ('OFF', 'A', 'B' or 'INT').
    jumpto : int
        The sequence step to jump to. 0 or -1 is interpreted as next step.
    ...
    step : int
        Sequence step to be edited.
    flags_t : list, optional
        List of flag trigger specifiers to be active during this sequence step, if both options are
        selected, the flag is set to trigger (PULS).
    flags_h : list, optional
        List of flag high specifiers to be active during this sequence step.
    ...
    def make_sequence_continuous(self, sequencename):
        """
    Make the specified sequence repeat continuously.
    ...
    final_step : str
        Step to jump to. Possible options are:
        - 'FIRST': This enables the sequencer to jump to the first step in the sequence.
    ...
    def _is_output_on(self):
        """
    Asks the AWG if the output is enabled, i.e., if the AWG is running.
    ...
    def _get_filenames_on_device(self):
        """
    Returns a list of filenames found in <ftproot>\\waves.
    ...
    def _write_wfmx(self, filename, analog_samples, marker_bytes, is_first_chunk, is_last_chunk,
                    total_number_of_samples):
        """
    Appends a sampled chunk of a whole waveform to a wfmx-file. Creates the file 
    if it is the first chunk. If both flags (is_first_chunk, is_last_chunk) are set 
    to TRUE, it means that the whole ensemble is written as a whole in one big chunk.

    Parameters
    ----------
    name : str
        Represents the name of the sampled ensemble.
    analog_samples : dict
        Contains float32 numpy ndarrays, which are the samples for the analog channels 
        that are to be written by this function call.
    marker_bytes : np.ndarray
        Contains bool numpy ndarrays, which are the samples for the digital channels 
        that are to be written by this function call.
    total_number_of_samples : int
        The total number of samples in the entire waveform. Must be known in advance.
    is_first_chunk : bool
    ...
    -------
    list
        The list contains the string names of the created files for the passed 
        presampled arrays.
    """
