...
class SwitchDummy(SwitchInterface):
    ...
    def on_activate(self):
        ...
        if (
            ...
            self._states = {
                switch: states[0] for switch, states in self._switches.items()
            }
    ...
    def name(self):
        """
        Get the name of the hardware as a string.
    ...
    def available_states(self):
        """
        Get the names of the states as a dictionary of tuples.
    ...
    def set_state(self, switch, state):
        """
        Query and set the state of a single switch by name.
        ...
        state : str
            Name of the state to set for the switch.
        """

        avail_states = self.available_states
        assert switch in avail_states, f'Invalid switch name: "{switch}"'
        assert (
            state in avail_states[switch]
        ), f'Invalid state name "{state}" for switch "{switch}"'
