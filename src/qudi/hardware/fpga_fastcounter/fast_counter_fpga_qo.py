...
class FastCounterFPGAQO(FastCounterInterface):
    """
    Hardware class for the Spartan-6 (Opal Kelly XEM6310) FPGA-based fast counter.

    The command reference for communicating via the OpalKelly Frontend can be found here:

        https://library.opalkelly.com/library/FrontPanelAPI/index.html

    The Frontpanel is essentially a C++ interface, where a wrapper was used (SWIG) to access the
    DLL library. Be aware that the wrapper is specified for a specific version of Python (here
    Python 3.4), and it is not guaranteed to work with other versions.
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._binwidth = (
            -1
        )  # number of elementary bins to be combined into a single bin
    ...
    def _read_status_register(self):
        """
        Read the 32-bit status register from the FPGA Timetagger via USB.
    ...
    def _set_dac_voltages(self):
        ...
        with self.threadlock:
            ...
            for dac_chnl in range(8):
                ...
                dac_value = int(
                    np.rint(4096 * self._switching_voltage[sma_chnl] / (2.5 * 2))
                )
                if dac_value > 4095:
                    dac_value = 4095
                elif dac_value < 0:
    ...
    def configure(self, bin_width_s, record_length_s, number_of_gates=0):
        ...
        tuple
            ...
            - number_of_gates : int
                The number of gated inputs that are accepted.
        """
    ...
    def get_data_trace(self):
        """
        Poll the current time trace data from the fast counter.

        Returns
        -------
        numpy.ndarray
            A 2-dimensional numpy array. This counter is gated, and the return array has the following shape:
            `returnarray[gate_index, timebin_index]`.

        Notes
        -----
        The binning, specified by calling `configure()` beforehand, must be handled in this hardware class.
        Any possible overflow of the histogram bins must be managed here.
        """
