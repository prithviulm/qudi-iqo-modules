...
If you are not sure about the meaning, refer to other hardware files for context. 
If additional constraints are needed, they must be added to the PulserConstraints class.

Each scalar parameter is a ScalarConstraints object defined in cor.util.interfaces.
Essentially, it contains min/max values as well as min step size, default value, and unit of
the parameter.

PulserConstraints.activation_config differs, as it contains the channel
...
workspace into the channel. For a device without mass memory, this will make the waveform/pattern 
...
load_dict : dict or list
    ...
    If just a list of waveform names is given, the channel association will be invoked from the 
    channel suffix '_ch1', '_ch2', etc.
...
"""
    ...
    def load_sequence(self, sequence_name):
        """
Load a sequence to the channels of the device in order to be ready for playback.
...
The returned dictionary will have the channel numbers as keys. In the case of loaded waveforms, 
the dictionary values will be the waveform names. In the case of a loaded sequence, the values 
will be the sequence name appended by a suffix representing the track loaded to the respective 
channel (i.e., '<sequence_name>_1').

Returns
-------
tuple
    - dict
        Dictionary with keys being the channel number and values being the respective asset 
...
tuple
    - int
        Integer value of the current status.
    - dict
        Corresponding dictionary containing status descriptions for all possible status 
...
Notes
-----
After setting the sampling rate of the device, use the actual set return value for 
further processing.
"""
...
amplitude : list, optional
    If the amplitude value (in Volt peak to peak, i.e., the full amplitude) of a specific 
    channel is desired.
...
tuple
    - dict
        Dictionary with keys being the channel descriptor strings (i.e., 'a_ch1') and items 
        being the values for those channels. Amplitude is always denoted in Volt-peak-to-peak 
        and Offset in volts.
    - dict
        Dictionary with keys being the channel descriptor strings (i.e., 'a_ch1') and items 
        being the offset values for those channels.
...
"""
    ...
    def set_analog_level(self, amplitude=None, offset=None):
        """
Set the amplitude and/or offset value of the provided analog channel(s).

Parameters
----------
amplitude : dict
    Dictionary with keys being the channel descriptor strings (i.e., 'a_ch1', 'a_ch2') and 
    items being the amplitude values (in Volt peak to peak, i.e., the full amplitude) for 
    the desired channels.
offset : dict
    Dictionary with keys being the channel descriptor strings (i.e., 'a_ch1', 'a_ch2') and 
    items being the offset values (in absolute volts) for the desired channels.

Returns
-------
tuple
    - dict
        Dictionary with the actual set values for amplitude for all channels.
    - dict
        Dictionary with the actual set values for offset for all channels.

Notes
-----
If nothing is passed, then the command will return the current amplitudes/offsets.

After setting the amplitude and/or offset values of the device, use the actual set 
...
"""
    ...
    def get_digital_level(self, low=None, high=None):
        """
Retrieve the digital low and high levels of the provided/all channels.
...
tuple
    - dict
        Dictionary with keys being the channel descriptor strings (i.e., 'd_ch1', 'd_ch2') 
        and items being the low values for those channels, denoted in volts.
    - dict
        Dictionary with keys being the channel descriptor strings (i.e., 'd_ch1', 'd_ch2') 
        and items being the high values for those channels, denoted in volts.

Notes
-----
Do not return a saved low and/or high value but instead retrieve the current low and/or high 
value directly from the device.

If nothing (or None) is passed, then the levels of all channels will be returned. If no 
digital channels are present, return just an empty dict.

Example of a possible input:
    low = ['d_ch1', 'd_ch4']
To obtain the low voltage values of digital channels 1 and 4. A possible answer might be:
...
"""
    ...
    def set_digital_level(self, low=None, high=None):
        """
Set the low and/or high value of the provided digital channel.

Parameters
----------
low : dict
    Dictionary with keys being the channel descriptor strings (i.e., 'd_ch1', 'd_ch2') and 
    items being the low values (in volts) for the desired channels.
high : dict
    Dictionary with keys being the channel descriptor strings (i.e., 'd_ch1', 'd_ch2') and 
    items being the high values (in volts) for the desired channels.

Returns
-------
tuple
    - dict
        Dictionary denoting the current low values for all digital channels, with keys being 
        the channel descriptor strings (i.e., 'd_ch1', 'd_ch2').
    - dict
        Dictionary denoting the current high values for all digital channels, with keys being 
        the channel descriptor strings (i.e., 'd_ch1', 'd_ch2').
...
After setting the high and/or low values of the device, use the actual set return values for 
further processing.
"""
...
----------
ch : list, optional
    If specific analog or digital channels are needed to be queried without obtaining all 
    the channels.

Returns
-------
dict
    Dictionary where keys denote the channel strings and items are boolean expressions 
    indicating whether the channel is active or not.

Notes
-----
Example of a possible input (order is not important):
...
(True: active, False: inactive). The actually set and returned channel 
activation must be part of the available activation_configs in the constraints. 
...
If the resulting set of active channels cannot be found in the available 
...
After setting the active channels of the device, use the returned dict for further 
processing.

Example of possible input:
    ch = {'a_ch2': True, 'd_ch1': False, 'd_ch3': True, 'd_ch4': True}
To activate analog channel 2, digital channels 3 and 4, and to deactivate 
...
"""
    ...
    def write_waveform(self, name, analog_samples, digital_samples, is_first_chunk, is_last_chunk,
        ...
        """
Write a new waveform or append samples to an already existing waveform in the device memory.

The flags `is_first_chunk` and `is_last_chunk` can be used as indicators to determine if a new 
waveform should be created or if the write process to a waveform should be terminated.
...
----------
name : str
    The name of the waveform to be created/appended to.
analog_samples : dict
    Keys are the generic analog channel names (i.e., 'a_ch1') and values are 1D numpy arrays 
    of type float32 containing the voltage samples.
...
is_first_chunk : bool
    Flag indicating if it is the first chunk to write. If True, this method will create a new 
    empty waveform. If False, the samples are appended to the existing waveform.
is_last_chunk : bool
    Flag indicating if it is the last chunk to write. Some devices may need to know when to 
    close the appending waveform.
...
tuple
    - int
        Number of samples written (-1 indicates a failed process).
    - list
        List of created waveform names.
"""
    ...
    def write_sequence(self, name, sequence_parameters):
        """
Write a new sequence to the device memory.
...
sequence_parameters : dict
    Dictionary containing the parameters for a sequence.
...
"""
    ...
    def get_interleave(self):
        """
Check whether interleave is ON or OFF in the AWG.
...
Notes
-----
After setting the interleave of the device, retrieve the interleave again 
and use that information for further processing.

This function is unused for pulse generator hardware other than an AWG.
"""
