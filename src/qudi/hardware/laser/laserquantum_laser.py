...
class LaserQuantumLaser(SimpleLaserInterface):
    ...
    serial_interface = ConfigOption(
        name="interface", default="ASRL1::INSTR", missing="warn"
    )
    ...
    def set_control_mode(self, mode):
        ...
        if self.psu not in (PSUTypes.SMD6000, PSUTypes.SMD12):
            if mode == ControlMode.POWER:
                ...
                self.log.debug(
                    "Set POWER control mode {0}, {1}.".format(reply1, reply2)
                )
            else:
                reply1 = self.inst.query("PFB=ON")
                reply2 = self.inst.query("CONTROL=CURRENT")
                self.log.debug(
                    "Set CURRENT control mode {0}, {1}.".format(reply1, reply2)
                )
    ...
    def get_psu_temperature(self):
        """
        Get the power supply temperature.
    ...
    def get_temperatures(self):
        ...
        -------
        dict
            Dictionary of temperature names and values.
        """
    ...
    def get_firmware_version(self):
        """
        Ask the laser for its ID.

        Returns
        -------
        str
            Information that the laser provides about itself.
        """
    ...
    def dump(self):
        ...
        -------
        str
            Diagnostic information dump from the laser.
        """
    ...
    def get_extra_info(self):
        """
        Extra information from the laser.
        For LaserQuantum devices, this includes the firmware version, dump, and timers information.

        Returns
        -------
        str
            Multiple lines of text with information about the laser.
        """
